...
std::pair<size_t,double*> Mandelbrot::calcOptimized(const double start_x,const double end_x,const double start_y,const double end_y,unsigned int max_iter){
    points.clear();
    // рассчет шага
    ...
    for (double x=start_x;x<=end_x;x+=step_x){
        for (double y=start_y;y<=end_y;y+=step_y){
            ...
            double im{0},real{0};
            unsigned int i=0;
            while (i<max_iter && (pow(real,2)+pow(im,2))<=4){
                //cn=square(cn)+Complex(x,y);
                double oldReal=real;
                real=pow(real,2)-pow(im,2)+x;
                im=2*im*oldReal+y;

                ++i;
                ++iterNumb;
            }
            if (pow(real,2)+pow(im,2)<=4){
                points.push_back(x);
                points.push_back(y);
            }
        }
